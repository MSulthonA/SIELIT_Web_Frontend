  ...
  function login(e: any) {
    ...
    if (!loginData.name || !loginData.password) {
      ...
      axios
        ...
        .catch((err: any) => {
          if (err.response) {
            // Jika ada response dari server, tangani berdasarkan status
            if (err.response.status === 401) {
              localStorage.setItem("token", "");
              setToken("");
            } else {
              toast.error(err.response.data?.msg || "Terjadi kesalahan pada server", { theme: "colored" });
            }
          } else {
            // Jika tidak ada response (kesalahan jaringan, server down, dll)
            toast.error("Gagal menghubungi server. Periksa koneksi internet Anda.", { theme: "colored" });
          }
        });
    }
  }
