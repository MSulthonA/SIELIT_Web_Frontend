...
function BypassPresensi() {
  ...
  function getAttendances() {
    axios
      .get(`${appSettings.api}/attendances?classId=${selectedClassId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <div className="min-h-[100svh] flex flex-col items-center justify-start pt-4 pb-16 grow w-full overflow-x-scroll">
      <p className="font-bold text-xl md:text-3xl mb-16">
        Bypass <span className="text-themeTeal">Presensi</span>
      </p>
              ...
              return (
                checkSearch(attendance) && (
                  <tr className="even:bg-slate-200 odd:bg-white" key={index}>
                    ...
                    <td>
                      <div className={`rounded ${attendance.status ? (attendance.status === "hadir" ? "bg-green-500/70	 text-white " : "bg-yellow-500/70 text-white ") : "bg-red-500/70 text-white"}`}>
