...
function JadwalKelas() {
  ...
  return (
    <div className="flex flex-col items-center justify-start relative text-lg">
      <p className="font-bold text-3xl md:text-3xl mb-16">
        Jadwal <span className="text-themeTeal">Kelas</span>
      </p>
      <div className="w-10/12 flex flex-col md:w-full md:flex-row md:gap-8 justify-between items-end md:items-center mb-4 md:max-w-6xl">
        ...
        <button className="bg-themeTeal self-end md:self-start md:mt-1 text-white text-sm font-semibold px-4 py-2 mt-3 h-fit rounded" onClick={getUpcomingClasses}>
          Terapkan filter
        </button>
      </div>
                ...
                checkSearch(upcomingClass) && (
                  <tr className="even:bg-slate-200 odd:bg-white" key={index}>
                    ...
                    <td className="px-6 py-2">
                      {namaHari[startDate.getDay()]}, {startDate.toLocaleString("id").replace(/\//g, "-").replace(",", "").split(" ")[0]} {startDate.toLocaleTimeString("id")} s/d {endDate.toLocaleTimeString("id")}
                    </td>
                  </tr>
                )
              );
